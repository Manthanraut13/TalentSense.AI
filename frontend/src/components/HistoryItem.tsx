...
export function HistoryItem({ item, onDelete, isActive, analysisId }: HistoryItemProps) {
  ...
  const scoreColor = scoreColorClass(item.scores.overall);

  return (
    <div
      data-analysis-id={analysisId}
      className={`relative group rounded-xl border border-line bg-surface p-4 shadow-card transition hover:shadow-cardHover ${
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <div className="mt-2 flex items-center gap-3 text-xs text-textSecondary">
              <span className="flex items-center gap-1">
                <span className={scoreColorClass(item.scores.skills_match)}>Skills: {item.scores.skills_match}%</span>
              </span>
              <span className="flex items-center gap-1">
                <span className={scoreColorClass(item.scores.experience_relevance)}>Exp: {item.scores.experience_relevance}%</span>
              </span>
              <span className="flex items-center gap-1">
                <span className={scoreColorClass(item.scores.keyword_coverage)}>KW: {item.scores.keyword_coverage}%</span>
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={deleting}
              className="flex h-8 w-8 items-center justify-center rounded-md text-textSecondary transition-colors hover:bg-red-500/10 hover:text-red-600 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-base"
